defmodule RaftDB.Raft.Communication.LeaderHook do
  @moduledoc """
  ...
  @doc """
  Hook to be called when a command submitted by `RaftDB.Raft.Node.command/4` is committed.
  """
  ...
  @doc """
  Hook to be called when a query given by `RaftDB.Raft.Node.query/3` is executed.
  """
  ...
  by `RaftDB.Raft.Node.start_link/2` with `:join_existing_consensus_group` specified.
  """
  @callback on_follower_added(Statable.data(), pid) :: neglected

  @doc """
  Hook to be called when a follower is removed from a consensus group by `RaftDB.Raft.Node.remove_follower/2`.
  """
...
defmodule RaftDB.Raft.Communication.LeaderHook.NoOp do
  @moduledoc """
