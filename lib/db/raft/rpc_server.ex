...
defmodule RaftDB.Raft.RPCServer do
  @moduledoc """
  ...
  alias RaftDB.Raft.Communication.{Election, Leadership, Members}
  alias RaftDB.Raft.Log.Entry
  alias RaftDB.Raft.Persistence.{Logs, Snapshot, SnapshotMetadata, Store}
  alias RaftDB.Raft.Types.{CommandResults, Config, TermNumber}
  alias RaftDB.Raft.Types.Error.AddFollowerError
  ...
  defunp initialize_leader_state(config :: Config.t(), options :: [RaftDB.Raft.Node.option()]) ::
           State.t() do
  ...
  defp recover_state_from_snapshot_and_log(
    ...
    # In this case we neglect `config` given in the argument to `RaftDB.Raft.Node.start_link/2`.
    ...
    Process.put(:raft_db_raft_rpc_server_restoring, true)
    ...
    Process.delete(:raft_db_raft_rpc_server_restoring)
  ...
  defunp initialize_follower_state(
           ...
           options :: [RaftDB.Raft.Node.option()]
         ) :: State.t() do
    %Snapshot{
      members: members,
      term: term,
    ...
    case Keyword.get(options, :test_inject_fault_after_add_follower) do
      :raise ->
        raise AddFollowerError,
          message:
            "simulated error in RaftDB.Raft.RPCServer.call_add_server_with_fault_injection/2",
          pid: self()
