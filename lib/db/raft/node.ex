...
defmodule RaftDB.Raft.Node do
  @moduledoc """
  Public interface functions of `RaftDB.Raft` implementation.
  """

  alias RaftDB.Raft.Communication.LeaderHook.NoOp
  alias RaftDB.Raft.Communication.RemoteMessageGateway
  alias RaftDB.Raft.Persistence.Store
  alias RaftDB.Raft.RPCServer
  alias RaftDB.Raft.StateMachine.Statable
  alias RaftDB.Raft.Types.{CommandId, Config, LogIndex, TermNumber}
  ...
  - `{:create_new_consensus_group, config}`: Creates a new consensus group using the given `RaftDB.Raft.Types.Config.Config.t`.
  ...
  - `:persistence_dir`: Directory path in which both Raft logs and periodic snapshots are stored.
    ...
    Note that it's caller's responsibility to ensure that the specified directory is not used by other `RaftDB.Raft` process.
  ...
  - `:log_file_expansion_factor`: A number that adjusts when to make a snapshot file.
    ...
    `RaftDB.Raft` keeps track of the file sizes of "currently growing log file" and "previous snapshot file".
  ...
  `RaftDB.Raft` implements state recovery using log and snapshot files.
  ...
  @doc """
  Make a new instance of `RaftDB.Raft.Types.Config` struct.

  `data_module` must be an implementation of `RaftDB.Raft.StateMachine.Statable` behaviour.
  Available options:

  - `leader_hook_module`: An implementation of `RaftDB.Raft.Communication.LeaderHook`. Defaults to `RaftDB.Raft.Communication.LeaderHook.NoOp`.
  - `communication_module`: A module to define member-to-member async communication (`cast/2` and `reply/2`).
    Defaults to `RaftDB.Raft.Communication.RemoteMessageGateway`.
