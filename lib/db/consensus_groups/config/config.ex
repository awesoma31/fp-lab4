...
defmodule RaftDB.ConsensusGroups.Config.Config do
  @moduledoc """
  ...
  - The actual value used is obtained by
    `Application.get_env(:raft_db, :balancing_interval, #{@default_balancing_interval})`,
  ...
  - The actual value used is obtained by
    `Application.get_env(:raft_db, :leader_pid_cache_refresh_interval, #{@default_leader_pid_cache_refresh_interval})`,
  ...
  - The actual value used is obtained by
    `Application.get_env(:raft_db, :follower_addition_delay, #{@default_follower_addition_delay})`.
  """
  ...
  - A node is considered "unhealthy" if it has been disconnected from the other nodes
    without declaring itself as `inactive` (by calling `RaftDB.ConsensusGroups.Manager.deactivate/0`).
    RaftDB tries to reconnect to unhealthy nodes in order to recover from short-term issues
    such as temporary network failures (see also `:node_purge_reconnect_interval` below).
    To handle longer-term issues, RaftDB automatically removes nodes that remain "unhealthy"
  ...
  - The actual value used is obtained by
    `Application.get_env(:raft_db, :node_purge_failure_time_window, #{@default_node_purge_failure_time_window})`,
  ...
  - The actual value used is obtained by
    `Application.get_env(:raft_db, :node_purge_reconnect_interval, #{@default_node_purge_reconnect_interval})`,
  ...
  defun per_member_options() :: nil | module do
    Application.get_env(:raft_db, :per_member_options)
  end

  defun raft_config() :: nil | module do
    Application.get_env(:raft_db, :raft_config)
  end
end
