defmodule RaftDB.Raft.NodeTest do
  @moduledoc """
  Testing Raft Node and RPCServer
  """
  use ExUnit.Case

  alias RaftDB.Raft.RPCServer

  alias RaftDB.Raft.Types.RPC.{
    AppendEntriesRequest,
    AppendEntriesResponse,
    InstallSnapshot
  }

  alias RaftDB.Raft.Utils.Collections.PidSet
  ...
  @conf RaftDB.Raft.Node.make_config(SimpleInt,
  ...
  defp add_follower(leader, name \\ nil, dir \\ nil) do
    {:ok, follower} =
      RaftDB.Raft.Node.start_link({:join_existing_consensus_group, [leader]},
        name: name,
        persistence_dir: dir
      )
  ...
  defp make_cluster(n_follower, config \\ @conf, persist? \\ false) do
    ...
    {:ok, leader} =
      RaftDB.Raft.Node.start_link({:create_new_consensus_group, config}, persistence_dir: dir)
  ...
  test "should appropriately start/add/remove/stop server" do
    {:ok, leader} =
      RaftDB.Raft.Node.start_link({:create_new_consensus_group, @conf}, name: :foo)
    ...
    {:ok, follower2} =
      RaftDB.Raft.Node.start_link({:join_existing_consensus_group, [follower1, leader]},
        name: :baz
      )
    ...
    assert RaftDB.Raft.Node.remove_follower(leader, follower1) == :ok
    wait_until_member_change_completes(leader)
    assert_receive({:EXIT, ^follower1, :normal})
    # for coverage: wait until :remove_follower log entry is applied in follwer2
    :timer.sleep(100)

    assert RaftDB.Raft.Node.remove_follower(leader, follower2) == :ok
    wait_until_member_change_completes(leader)
  ...
  test "should not concurrently execute multiple membership changes" do
    {:ok, leader} = RaftDB.Raft.Node.start_link({:create_new_consensus_group, @conf})
  ...
  test "should report error when trying to remove leader" do
    ...
    assert RaftDB.Raft.Node.remove_follower(leader, leader) == {:error, :cannot_remove_leader}
  end

  test "should report error when trying to remove non member" do
    {:ok, leader} = RaftDB.Raft.Node.start_link({:create_new_consensus_group, @conf})
    assert RaftDB.Raft.Node.remove_follower(leader, self()) == {:error, :not_member}
  end
  ...
  test "should refuse to remove healthy follower if it breaks the current quorum" do
    ...
    assert RaftDB.Raft.Node.remove_follower(leader, follower2) == {:error, :will_break_quorum}
  end

  test "should remove target follower from unresponsive_follower list" do
    {leader, [follower1]} = make_cluster(1)
    assert RaftDB.Raft.Node.remove_follower(leader, follower1) == :ok
    assert RaftDB.Raft.Node.status(leader)[:unresponsive_followers] == []
    :timer.sleep(@t_max_election_timeout)
  ...
  test "should unset uncommitted_membership_change when log entry for add_follower/remove_follower is overwritten" do
    ...
    catch_exit(RaftDB.Raft.Node.remove_follower(m1, m2))
    assert RaftDB.Raft.Node.status(m2).leader in [m2, m3]
    CommunicationWithNetsplit.set([])
    wait_until_state_name_changes(m1, :follower)
    assert RaftDB.Raft.Node.status(m1).leader in [m2, m3]
  ...
  test "replace_leader should eventually replace leader" do
    ...
    assert RaftDB.Raft.Node.replace_leader(leader, leader) == {:error, :already_leader}

    assert RaftDB.Raft.Node.replace_leader(leader, follower1) == :ok
    wait_until_state_name_changes(leader, :follower)
    wait_until_state_name_changes(follower1, :leader)
    assert RaftDB.Raft.Node.command(leader, :inc) == {:error, {:not_leader, follower1}}
    assert RaftDB.Raft.Node.command(follower1, :inc) == {:ok, 0}

    assert RaftDB.Raft.Node.replace_leader(follower1, follower2) == :ok
    wait_until_state_name_changes(follower1, :follower)
    wait_until_state_name_changes(follower2, :leader)
    assert RaftDB.Raft.Node.command(follower1, :inc) == {:error, {:not_leader, follower2}}
    assert RaftDB.Raft.Node.command(follower2, :inc) == {:ok, 1}
  end
  ...
  test "replace_leader should reject change to unhealthy follower" do
    ...
    assert RaftDB.Raft.Node.replace_leader(leader, follower1) ==
  ...
  test "leader should respond to client requests" do
    ...
    assert RaftDB.Raft.Node.command(leader, :inc) == {:ok, 0}
    assert RaftDB.Raft.Node.command(leader, :inc) == {:ok, 1}
    assert RaftDB.Raft.Node.command(leader, :inc) == {:ok, 2}
    assert RaftDB.Raft.Node.command(leader, :inc) == {:ok, 3}
    assert RaftDB.Raft.Node.command(leader, :get) == {:ok, 4}
    assert RaftDB.Raft.Node.command(leader, {:set, 1}) == {:ok, 4}
    assert RaftDB.Raft.Node.command(leader, :get) == {:ok, 1}
    assert RaftDB.Raft.Node.command(leader, :inc) == {:ok, 1}
    assert RaftDB.Raft.Node.command(leader, :inc) == {:ok, 2}
    assert RaftDB.Raft.Node.command(leader, :get) == {:ok, 3}
    assert RaftDB.Raft.Node.query(leader, :get) == {:ok, 3}

    assert RaftDB.Raft.Node.command(follower1, :get) == {:error, {:not_leader, leader}}
    assert RaftDB.Raft.Node.query(follower1, :get) == {:error, {:not_leader, leader}}
    assert RaftDB.Raft.Node.command(follower2, :get) == {:error, {:not_leader, leader}}
    assert RaftDB.Raft.Node.query(follower2, :get) == {:error, {:not_leader, leader}}
  end
  ...
  test "read-only query should be handled in the same way as command when election_timeout_clock_drift_margin is large" do
    ...
    assert RaftDB.Raft.Node.command(leader, :inc) == {:ok, 0}
    assert RaftDB.Raft.Node.command(leader, :get) == {:ok, 1}
    assert RaftDB.Raft.Node.query(leader, :get) == {:ok, 1}
  ...
  test "all members should answer query_non_leader" do
    ...
    assert RaftDB.Raft.Node.query_non_leader(leader, :get) == {:ok, 0}
    assert RaftDB.Raft.Node.query_non_leader(follower1, :get) == {:ok, 0}
    assert RaftDB.Raft.Node.query_non_leader(follower2, :get) == {:ok, 0}
  ...
  test "should not execute the same client requests with identical reference multiple times" do
    ...
    assert RaftDB.Raft.Node.command(leader, :get) == {:ok, 0}
    assert RaftDB.Raft.Node.command(leader, {:set, 1}) == {:ok, 0}
    assert RaftDB.Raft.Node.command(leader, {:set, 2}) == {:ok, 1}
    assert RaftDB.Raft.Node.command(leader, :get) == {:ok, 2}

    ref = make_ref()
    assert RaftDB.Raft.Node.command(leader, :inc, 5000, ref) == {:ok, 2}
    assert RaftDB.Raft.Node.command(leader, :inc, 5000, ref) == {:ok, 2}
    assert RaftDB.Raft.Node.command(leader, :inc, 5000, ref) == {:ok, 2}
  ...
  test "1-member cluster should immediately respond to client requests" do
    ...
    :timer.sleep(@conf.election_timeout)
    assert RaftDB.Raft.Node.command(leader, :inc) == {:ok, 1}
    assert RaftDB.Raft.Node.query(leader, :get) == {:ok, 2}
    :timer.sleep(@conf.election_timeout)
    assert RaftDB.Raft.Node.command(leader, :inc) == {:ok, 2}
    assert RaftDB.Raft.Node.query(leader, :get) == {:ok, 3}
  end
  ...
  test "3,4,5,6,7 member cluster should tolerate up to 1,1,2,2,3 follower failure" do
    ...
    |> Enum.each(fn n_members ->
      ...
      assert RaftDB.Raft.Node.command(leader, :get, 50) == {:error, :timeout}
      # read-only query succeeds if it is processed within leader's lease
      assert RaftDB.Raft.Node.query(leader, :get, 50) == {:ok, 1}
  ...
  test "3,4,5,6,7 member cluster should elect new leader after leader failure" do
    ...
    |> Enum.each(fn n_members ->
      ...
      assert RaftDB.Raft.Node.command(new_leader, :get) == {:ok, 0}
      assert RaftDB.Raft.Node.query(new_leader, :get) == {:ok, 0}
    end)
  end
  ...
  test "should reject vote request from disruptive ex-member as long as leader is working fine" do
    defmodule DropRemoveFollowerCompleted do
      ...
      """
      @behaviour RaftDB.Raft.Communication.Communicable
  ...
  test "force_remove_member/2 should recover a failed consensus group" do
    ...
    |> Enum.each(fn n_members ->
      ...
      refute Enum.any?(remaining_members, fn member ->
               RaftDB.Raft.Node.status(member).state_name == :leader
             end)

      for f <- failed_members, r <- remaining_members do
        assert RaftDB.Raft.Node.force_remove_member(r, f) == :ok
      end
  ...
  test "after repairing by force_remove_member/2, leader should continue working without stepping down by itself" do
    ...
    :gen_statem.stop(f3)
    :timer.sleep(@t_max_election_timeout * 2)
    :ok = RaftDB.Raft.Node.force_remove_member(l0, f3)
    :ok = RaftDB.Raft.Node.force_remove_member(f1, f3)
    :timer.sleep(@t_max_election_timeout * 2)

    # Now 2/3 are alive, so there must be an established leader.
    new_leader = RaftDB.Raft.Node.status(l0).leader
    assert is_pid(new_leader)
    assert RaftDB.Raft.Node.status(f1).leader == new_leader

    Enum.each(1..30, fn _ ->
      :timer.sleep(200)
      assert RaftDB.Raft.Node.status(l0).leader == new_leader
      assert RaftDB.Raft.Node.status(f1).leader == new_leader
    end)
  end
  ...
  test "change_config should replace current config field on commit" do
    ...
    Enum.each(members, fn member ->
      assert RaftDB.Raft.Node.status(member).config == @conf
    end)

    new_conf = Map.update!(@conf, :election_timeout, fn t -> t + 1 end)
    assert RaftDB.Raft.Node.change_config(leader, new_conf) == :ok
    :timer.sleep(@conf.heartbeat_timeout * 2)

    Enum.each(members, fn member ->
      assert RaftDB.Raft.Node.status(member).config == new_conf
    end)
  end
  ...
  test "other callbacks just do irrelevant things" do
    ...
    result = RaftDB.Raft.Node.status(leader)
  ...
  test "leader should directly answer queries if leader lease is valid; should not if expired" do
    ...
    {:ok, leader} = RaftDB.Raft.Node.start_link({:create_new_consensus_group, config})
    follower1 = add_follower(leader)
    ...
    assert RaftDB.Raft.Node.query(leader, :get, 100) == {:error, :timeout}
  end

  test "lonely leader should reply query without making log entry" do
    {:ok, l} = RaftDB.Raft.Node.start_link({:create_new_consensus_group, @conf})
    {:leader, state1} = :sys.get_state(l)
    assert RaftDB.Raft.Node.query(l, :get) == {:ok, 0}
  ...
  defp call_client_request(member, {:command, arg}, ref) do
    RaftDB.Raft.Node.command(member, arg, 500, ref)
  end

  defp call_client_request(member, {:query, arg}, _ref) do
    RaftDB.Raft.Node.query(member, arg, 500)
  end
  ...
  def op_replace_leader(%{working: working, current_leader: leader} = context) do
    ...
    assert RaftDB.Raft.Node.replace_leader(leader, next_leader) == :ok
  ...
  def op_remove_follower(
    ...
    if 2 * length(followers_in_majority) > length(all_members) do
      ...
      assert RaftDB.Raft.Node.remove_follower(leader, target) == :ok
  ...
  defp assert_leader_status(leader, members, isolated) do
    ...
    s = RaftDB.Raft.Node.status(leader)
