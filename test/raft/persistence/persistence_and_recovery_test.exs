defmodule RaftDB.Raft.Persistence.PersistenceAndRecoveryTest do
  @moduledoc """
  ...
  alias RaftDB.Raft.Persistence.{Snapshot, Store}

  @config RaftDB.Raft.Node.make_config(SimpleInt, leader_hook_module: MessageSendingHook)
  @name :foo
  @tmp_dir "tmp"
  ...
  test "persist logs and snapshots and recover by reading them" do
    {:ok, _} =
      RaftDB.Raft.Node.start_link({:create_new_consensus_group, @config},
        name: @name,
    ...
    assert RaftDB.Raft.Node.command(@name, :inc) == {:ok, 0}
    ...
    Enum.each(1..40, fn i ->
      assert RaftDB.Raft.Node.command(@name, :inc) == {:ok, i}
    end)
    ...
    assert RaftDB.Raft.Node.query(@name, :get) == {:ok, 41}
    assert :gen_statem.stop(@name) == :ok
  end
  ...
  test "uncommitted logs should be committed by lonely leader immediately after recovery" do
    ...
    {:ok, f} = RaftDB.Raft.Node.start_link({:join_existing_consensus_group, [l]})
    assert MapSet.new(RaftDB.Raft.Node.status(l).members) == MapSet.new([l, f])
    assert :gen_statem.stop(f) == :ok

    # Now incoming commands won't be committed
    Enum.each(1..3, fn _ ->
      assert RaftDB.Raft.Node.command(l, :inc, 100) == {:error, :timeout}
    end)
    ...
    assert RaftDB.Raft.Node.query(@name, :get) == {:ok, 3}
    assert :gen_statem.stop(@name) == :ok
  end
  ...
  test "follower should replicate log entries and store them in disk with de-duplication" do
    ...
    {:ok, f} =
      RaftDB.Raft.Node.start_link({:join_existing_consensus_group, [l]}, persistence_dir: dir_f)
    ...
    assert RaftDB.Raft.Node.command(l, :inc) == {:ok, 0}
  ...
  test "follower should reset its members field when recovery log entry is found" do
    ...
    {:ok, f1} =
      RaftDB.Raft.Node.start_link({:join_existing_consensus_group, [l1]},
        persistence_dir: dir_f
      )

    assert Enum.sort(RaftDB.Raft.Node.status(l1).members) == Enum.sort([l1, f1])
    assert Enum.sort(RaftDB.Raft.Node.status(f1).members) == Enum.sort([l1, f1])
    ...
    {:ok, f2} =
      RaftDB.Raft.Node.start_link({:join_existing_consensus_group, [l2]},
        persistence_dir: dir_f
      )

    assert Enum.sort(RaftDB.Raft.Node.status(l2).members) == Enum.sort([l2, f2])
    assert Enum.sort(RaftDB.Raft.Node.status(f2).members) == Enum.sort([l2, f2])
  ...
  test "non-persisting and persisting members can interchange snapshots with each other" do
    {:ok, n1} = RaftDB.Raft.Node.start_link({:create_new_consensus_group, @config})

    Enum.each(0..10, fn i ->
      assert RaftDB.Raft.Node.command(n1, :inc) == {:ok, i}
    end)

    # send snapshot: `n1` => `p1`
    {:ok, p1} =
      RaftDB.Raft.Node.start_link({:join_existing_consensus_group, [n1]},
        persistence_dir: @tmp_dir
      )
    ...
    Enum.each(21..30, fn i ->
      assert RaftDB.Raft.Node.command(p2, :inc) == {:ok, i}
    end)

    # send snapshot: `p2` => `n2`
    {:ok, n2} = RaftDB.Raft.Node.start_link({:join_existing_consensus_group, [p2]})
  ...
  test "older snapshots should be discarded when making a new one" do
    ...
    Enum.each(0..100, fn i ->
      assert RaftDB.Raft.Node.command(p1, :inc) == {:ok, i}
    end)
  ...
  test "should recover from the latest snapshot and logs" do
    ...
    {:ok, n} = RaftDB.Raft.Node.query(p1, :get)
    assert :gen_statem.stop(p1) == :ok
    ...
    assert RaftDB.Raft.Node.query(p2, :get) == {:ok, n}
    assert :gen_statem.stop(p2) == :ok
  end
  ...
  test "should correctly reset current consensus group members on successful election" do
    ...
    {:ok, f1} =
      RaftDB.Raft.Node.start_link({:join_existing_consensus_group, [l]},
    ...
    assert RaftDB.Raft.Node.status(l).leader == nil

    # Forcibly remove the dead pid to restore leader.
    assert RaftDB.Raft.Node.force_remove_member(l, f1) == :ok
    :timer.sleep(election_timeout_max)
    assert RaftDB.Raft.Node.status(l).leader == l
    ...
    # :timer.sleep(500)
    # :timer.sleep(@config.election_timeout * 2)

    wait_until(fn ->
      Enum.sort(RaftDB.Raft.Node.status(l).members) == Enum.sort([l, f2])
    end)

    assert Enum.sort(RaftDB.Raft.Node.status(l).members) == Enum.sort([l, f2])
    assert Enum.sort(RaftDB.Raft.Node.status(f2).members) == Enum.sort([l, f2])
