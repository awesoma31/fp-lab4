...
4. Когда приходит команда, например `put("x", 42)`:
...
Каждый RaftNode — это процесс с тремя состояниями:
...
В ходе выполнения лабораторной работы я познакомился с алгоритмом консенсуса Raft и реализовал его на Elixir, поработал с кластеризацией и node-to-node коммуникацией в ErlangVM, научился тестировать распределенные Elixir приложения.
